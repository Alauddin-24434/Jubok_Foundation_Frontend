      ...
      {isLoading ? (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {[...Array(6)].map((_, i) => (
            <div
              key={i}
              className="h-[400px] w-full bg-slate-100 animate-pulse rounded-xl"
            />
          ))}
        </div>
      ) : (
        <>
          {data?.data?.length > 0 ? (
            <>
              <div
                className={`grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 transition-opacity ${isFetching ? "opacity-50" : "opacity-100"}`}
              >
