...
export default function NoticePage() {
  ...
  const handlePdfUpload = async (file: File) => {
    ...
    try {
      ...
      toast.error("Cryptographic signature failed or network error", {
        id: toastId,
      });
    } finally {
      ...
      <AFPageHeader
        ...
        action={
          <Button
            ...
            className="rounded-xl shadow-lg h-11 px-6 font-black transition-all hover:scale-105 active:scale-95"
          >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {notices.length > 0 ? (
          notices.map((notice) => (
            <Card
              key={notice._id}
              className="group relative overflow-hidden border-2 border-muted/30 hover:border-primary/40 transition-all duration-300 p-6 bg-card/50 backdrop-blur-sm shadow-md hover:shadow-xl"
            >
              <div className="flex items-start gap-4">
                ...
                <div className="flex-1 space-y-1">
                  <h3 className="font-black text-foreground line-clamp-2 leading-tight tracking-tight">
                    {notice.title}
                  </h3>
          ...
          <div className="col-span-full py-32 text-center rounded-3xl border-2 border-dashed border-muted bg-muted/5 flex flex-col items-center gap-4">
            ...
            <div>
              ...
              <p className="text-sm text-muted-foreground/60 max-w-xs mx-auto">
                Click 'Publish Circular' to broadcast official documentation to
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div className="relative">
                ...
                {isUploading && (
                  <div className="absolute inset-0 bg-white/80 dark:bg-black/80 backdrop-blur-sm rounded-xl flex flex-col items-center justify-center gap-2">
                    ...
                    <p className="text-[10px] font-black text-primary uppercase animate-pulse">
                      Encoding Buffer...
                    </p>
                  </div>
                )}
          ...
          <div className="flex flex-col sm:flex-row justify-end gap-3 pt-6 border-t border-muted/30">
            <Button
              ...
              className="rounded-xl px-8 font-bold order-2 sm:order-1 h-11"
            >
              Cancel
            </Button>
            <Button
              ...
              className="rounded-xl px-10 font-black shadow-lg shadow-primary/20 h-11 order-1 sm:order-2"
            >
              Commit Circular
            </Button>
          </div>
